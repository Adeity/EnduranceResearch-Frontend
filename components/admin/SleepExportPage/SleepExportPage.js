...
class SleepExportPage extends React.Component{
    ...
    setResearchNumbersFromResponse(researchNumberFromResponse) {
        ...
        researchNumberFromResponse.forEach((e) => {
    ...
    handleExportButtonClick(e) {
        ...
        sendExportSleepsRequest(this.state.dateFrom, this.state.dateTo, checkedResearchNumbers)
            ...
            if (message !== null) {
                this.setState({errorMessage: message.errorMessage})
            } else {
    ...
    render() {
        ...
        const errorAlert = this.state.errorMessage !== null ? <div className={"alert alert-danger"}>{this.state.errorMessage}</div> : null;
        const loadingAlert = this.state.loading === true ? <div className={"alert alert-info"}>Probiha export</div> : null;
        return (
            <div className={"mx-auto w-md-75 "}>
                <h6>Export spánku</h6>
                <form>
